...
pub fn sign(sk: &[u8; LEN_PRIVATE_KEY], m: &[u8]) -> Result<[u8; SIG_LEN], MlDsaError> {
    #[cfg(feature="HEDGED")]
    {
        ...
        sign_message(sk, &rnd, &m)
    }

    #[cfg(feature="DETERMINISTIC")]
    sign_message(sk, &[0u8; 32], &m)
}

pub fn sign_message(sk: &[u8; LEN_PRIVATE_KEY], rnd: &[u8; 32], m: &[u8]) -> Result<[u8; SIG_LEN], MlDsaError> {
    sign_internal(sk, &m, &rnd)
}

pub fn sign_message_with_ctx(sk: &[u8; LEN_PRIVATE_KEY], rnd: &[u8; 32], m: &[u8], ctx: &[u8]) -> Result<[u8; SIG_LEN], MlDsaError> {
    if ctx.len() > 255 {
        Err(MlDsaError::SignCtxLenTooLong)
    } else {
...
fn sign_with_ctx_internal(sk: &[u8; LEN_PRIVATE_KEY], m: &[u8], rnd: &[u8; 32], ctx: Option<&[u8]>) -> Result<[u8; SIG_LEN], MlDsaError> {
    let (c_tilda, z, hint) = response_and_hint(sk, m, rnd, ctx)?;
    Ok(sig_encode(&c_tilda, &z, &hint))
}

pub fn response_and_hint(sk: &[u8; LEN_PRIVATE_KEY], m: &[u8], rnd: &[u8; 32], ctx: Option<&[u8]>) -> Result<([u8; LAMBDA/4], [[i32; N]; L], [[i32; N]; K]), MlDsaError> {
    // line 1
    ...
    for l in 0..L {
        s1_hat[l].copy_from_slice(&ntt(&s1[l]));
    }

    // line 3
    let mut s2_hat = [[0i32; N]; K];
    for k in 0..K {
        s2_hat[k].copy_from_slice(&ntt(&s2[k]));
    }

    // line 4
    let mut t0_hat = [[0i32; N]; K];
    for k in 0..K {
        t0_hat[k].copy_from_slice(&ntt(&t0[k]));
    }
...
fn high_bits(r: i32) -> i32 {
    ...
    let (_rr1, _rr0) = _ref_decompose_(r);
    assert_eq!(_rr1, r1);
    assert_eq!(_rr0, _r0);
    assert_eq!(mod_q(r as i64), mod_q((r1*2*GAMMA2 as i32 + _r0) as i64));
    r1
}
...
fn low_bits(r: i32) -> i32 {
    ...
    assert_eq!(mod_q(r as i64), mod_q((_r1*2*GAMMA2 as i32 + r0) as i64));
    r0
}

// r = rx mod Q
// r0 <- r mods(2*GAMMA2)
fn mods_2gamma2(rx: i32) -> i32 {
    const G2: i32 = GAMMA2 as i32;
    const TWICE_G2: i32 = 2 * GAMMA2 as i32;

    let r = rx.rem(TWICE_G2); // =  r % TWICE_G2
    if r > G2 {
        r-TWICE_G2
    } else {
        r
...
#[inline]
fn _ref_decompose_(rx: i32) -> (i32, i32) {
    ...
    let r0 = r - (r1 * 2 * GAMMA2 as i32);
    let r0 = r0 - ((((Q - 1)/2 - r0) >> 31) & Q);

    assert_eq!(mod_q(((r1 * (2*GAMMA2) as i32) + r0) as i64), mod_q(rx as i64));
